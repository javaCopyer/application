...
public abstract class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    /**
     * 计算工作日
     * param nowDate 当日计算开始日期
     * param day 工作天数
     * param exclude 排除在外的节假日期 yyyy-mm-dd
     * param contain 包含在工作日内的日期 yyyy-mm-dd
     * @throws Exception
     */
    public static Date workDay(String nowDate, int day, List<String> exclude, List<String> contain) throws Exception {
        // 创建一个日期实例
        Date nowD = DateUtils.GetFormatDate(DateUtils.FORMAT_2, nowDate);
        ...
        while (count < day) {
            ...
            String date = DateUtils.FormatDate("yyyy-MM-dd", cal.getTime());
            cal.add(Calendar.DAY_OF_MONTH, 1);
            if (contain.contains(date)) {
                count++;
            } else if (week == 0 || week == 6 || exclude.contains(date)) {
                continue;
            } else {
                count++;
            }
    ...
    public static String FormatDate(String fromFormat, long dateLong) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat(fromFormat);
        TimeZone zone = TimeZone.getTimeZone("GMT+8");
        sdf.setTimeZone(zone);
    ...
    public static String DateToString(Date date){
        String dateS;
        String fml="yyyy-MM-dd";
        SimpleDateFormat sdf=new SimpleDateFormat(fml);
        date =new Date();

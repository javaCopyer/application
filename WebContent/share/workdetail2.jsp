...
<%
	...
	String sharephone=request.getParameter("sharephone");
...
<html>
	...
	<body style="background: #fff;">
		...
		<script type="text/javascript">
				$(function(){
					var data={productid:"<%=productid %>",phone:"<%=sharephone %>"} 
					console.log(data);
					$.ajax({
				         type: "get",
				         url: "/api/method014",
				         ...
				         success: function(data) {
							...
							console.log(listinfo.lifecontent+"&&&&&&"+listinfo.memberamount)
							...
							if(listinfo.memberamount==0){ 
								...
								console.log(data.obj.用户信息.usertype)
							...
							$("#usercont").html("<p>"+data.obj.用户信息.name+"</p><%=sharephone %>")
							if(data.obj.用户信息.usertype!=8&&data.obj.用户信息.usertype!=9){
								$("#tomember").hide();
							}
			...
			$("#tomemberck").click(function(){
				if(window.usertype==8||window.usertype==9){
					window.location.href='/api/method063?userid='+window.userid+'&openid=<%=openid %>&sharephone=<%=sharephone %>';
				}
			})
		</script>
	</body>
